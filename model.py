...
class View(nn.Module):
    ...
    def forward(self, input):
        # print('Input_view_something: ',input.view(*self.shape))
...
class Net(nn.Module):
    ...
    def forward(self, x, landmarks_list): #landmarks list  
        ...
        landmarks_list[landmarks_list > 20] = 20
          
        patches_20 = [[out[i, :, x-3:x+3, y-3:y+3] for x, y in landmarks_list[i]] for i in range(bs)] 
        patches_20 = torch.stack([torch.stack(patches_20[i], dim=0) for i in range(bs)], dim=0) #bs x 24 x 512 x 6 x 6 
        local_features = torch.cat([self.pg_unit(patches_20[:, i], i) for i in range(20)], dim=1)
       
        global_features = self.gg_unit(out)    
        # print("Global Features: ",global_features.shape)
        # print('bs: ',bs)
        # print('Local Features:',local_features.shape)
        input_features = torch.cat((local_features, global_features), dim = 1)
        # print('Features: ', input_features.shape)
        f = self.fc1(input_features)       
        f = self.fc2(f) 
        # print(f.shape)
        return f 
