'''
Returns 20-landmarks points along with image and label for AffectNet dataset
...
def get_class(idx): #These are affectnet labels
        classes = {
            0: 'Neutral',
            1: 'Happy',
            2: 'Sad',
...
def change_emotion_label_same_as_affectnet(meta):
        """
        ...
        new_emo = 0
        # print("meta: ",meta)
        if meta == 0:  #Affectnet Labels mapped to CKplus labels
            new_emo = 0#
        elif meta == 1:
            new_emo = 1#
        elif meta == 2:
...
def default_reader(fileList, landmarksfile,num_classes):
    ...
    with open(fileList, 'r') as fp:
        for line in fp.readlines(): 
            # imgname= (line.rsplit('/',1)[1]).strip('\n')
            one_list.append(line)
    with open(landmarksfile, 'rb') as handle:
        unserialized_data = pkl.load(handle)
    
    for img_path in one_list:
        img_path = img_path.strip('\n')
        # print(img_path)
        # img_name= (img_path.rsplit('/',1)[1]).strip('\n')
        img_name= img_path
        if img_name in unserialized_data:
            if unserialized_data[img_name][1] == 8:
                continue
            elif unserialized_data[img_name][0] != 0:
                exp , landmarks_68 = unserialized_data[img_name][1], unserialized_data[img_name][0]
                # print('Before exp:',exp)
                exp = int(exp) - 1
...
# import pickle as pkl
# rootfolder= '/projects/joshi/datasets/affectnet_kaggle_masked'
# fileList = '/projects/joshi/datasets/affectnet_kaggle_masked/affectnet_trainList.txt'
# landmarksfile='/projects/joshi/datasets/affectnet_kaggle_masked/trainlandmarksfile.pkl'
...
if __name__=='__main__':
   ...
   transform = transforms.Compose([transforms.Resize((224, 224)),
                                           transforms.ToTensor()])
   dataset = ImageList(rootfolder, fileList,landmarksfile='/projects/joshi/datasets/affectnet_kaggle_masked/trainlandmarksfile.pkl',  transform=transform)
