'''
Returns 20 landmarks points along with image and label for RAF-DB dataset   
'''
...
def change_emotion_label_same_as_affectnet(meta):
        ...
        new_emo = 0
        # print("meta: ",meta)
        if meta == 1:  #RAF_DB Labels mapped to My Uniform Labels as in One Notes
            new_emo = 3  #
        elif meta == 2:  
            new_emo = 5  #
        elif meta == 3:  
...
def default_reader(fileList, landmarksfile,num_classes):
    ...
    for img_path in one_list:
        ...
        if img_path in unserialized_data:
            if int(unserialized_data[img_path][1]) > 8:
                continue
            elif unserialized_data[img_path][0] != 0:
...
'''
import pickle as pkl
rootfolder= '/projects/joshi/datasets/RAF-FaceDataset/original'
fileList = '/projects/joshi/projects/01_Unmasked_datasets/RAF-DB/raf_db_trainsList.txt'
landmarksfile='/projects/joshi/projects/01_Unmasked_datasets/RAF-DB/all_landmarksfile.pkl'
imgList,num_per_cls_dict =  default_reader(fileList,landmarksfile, num_classes=7)
